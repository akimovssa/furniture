...
namespace furniture
{
    public partial class MainForm : Form
    {
        ...
        private void InitializeComponents()
        {
            ...
            var btnExportPurchases = new Button
            {
                Text = "Экспорт",
                Dock = DockStyle.Top,
                Height = 30
            };
            btnExportPurchases.Click += (s, e) => ExportToExcel(dgvPurchases, "Закупки");
            var panelPurchases = new Panel { Dock = DockStyle.Top, Height = 120 };
            panelPurchases.Controls.AddRange(new Control[] { btnAddPurchase, btnEditPurchase, btnDeletePurchase, btnExportPurchases });
            ...
            btnExportSales.Click += (s, e) => ExportToExcel(dgvSales, "Продажи");
            var panelSales = new Panel { Dock = DockStyle.Top, Height = 120 };
            panelSales.Controls.AddRange(new Control[] { btnAddSale, btnEditSale, btnDeleteSale, btnExportSales });
            ...
            var btnExportSuppliers = new Button
            {
                Text = "Экспорт",
                Dock = DockStyle.Top,
                Height = 30
            };
            btnExportSuppliers.Click += (s, e) => ExportToExcel(dgvSuppliers, "Поставщики");
            var panelSuppliers = new Panel { Dock = DockStyle.Top, Height = 120 };
            panelSuppliers.Controls.AddRange(new Control[] { btnAddSupplier, btnEditSupplier, btnDeleteSupplier, btnExportSuppliers });
            ...
            var btnExportEmployees = new Button
            {
                Text = "Экспорт",
                Dock = DockStyle.Top,
                Height = 30
            };
            btnExportEmployees.Click += (s, e) => ExportToExcel(dgvEmployees, "Сотрудники");
            var panelEmployees = new Panel { Dock = DockStyle.Top, Height = 120 };
            panelEmployees.Controls.AddRange(new Control[] { btnAddEmployee, btnEditEmployee, btnDeleteEmployee, btnExportEmployees });
            ...
            var btnExportCategories = new Button
            {
                Text = "Экспорт",
                Dock = DockStyle.Top,
                Height = 30
            };
            btnExportCategories.Click += (s, e) => ExportToExcel(dgvProductCategories, "Категории товаров");
            var panelCategories = new Panel { Dock = DockStyle.Top, Height = 120 };
            panelCategories.Controls.AddRange(new Control[] { btnAddCategory, btnEditCategory, btnDeleteCategory, btnExportCategories });
            ...
            var btnExportProducts = new Button
            {
                Text = "Экспорт",
                Dock = DockStyle.Top,
                Height = 30
            };
            btnExportProducts.Click += (s, e) => ExportToExcel(dgvProducts, "Товары");
            var panelProducts = new Panel { Dock = DockStyle.Top, Height = 120 };
            panelProducts.Controls.AddRange(new[] { btnAddProduct, btnEditProduct, btnDeleteProduct, btnExportProducts });
            ...
            btnExportClients.Click += (s, e) => ExportToExcel(dgvClients, "Клиенты");
            var panelClients = new Panel { Dock = DockStyle.Top, Height = 120 };
            panelClients.Controls.AddRange(new[] { btnAddClient, btnEditClient, btnDeleteClient, btnExportClients });
...
2. Главное окно
   ...
   - Каждая вкладка отображает данные в таблице и предоставляет кнопки для добавления, редактирования, удаления и экспорта записей.
...
3. Работа с данными
   ...
   - Экспорт данных: Нажмите ""Экспорт"" для сохранения данных таблицы в Excel-файл (.xlsx).
...
5. Советы
   ...
   - Используйте экспорт для создания отчетов в Excel.
...
"
        ...
        private void ExportToExcel(DataGridView dataGridView, string sheetName)
        {
            ...
            if (saveDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    ...
                    if (excelApp == null)
                    {
                        throw new Exception("Microsoft Excel не установлен или не доступен.");
                    }
                    excelApp.Visible = false; // Скрываем Excel
                    workbook = excelApp.Workbooks.Add();
                    worksheet = (Excel.Worksheet)workbook.Sheets[1]; // Явное приведение к Worksheet
                    worksheet.Name = sheetName;

                    // Записываем заголовки
                    for (int i = 0; i < dataTable.Columns.Count; i++)
                    {
                        range = (Excel.Range)worksheet.Cells[1, i + 1]; // Явное приведение к Range
                        range.Value2 = dataTable.Columns[i].ColumnName;
                        range.Interior.Color = System.Drawing.ColorTranslator.ToOle(Color.LightGray);
                        range.Font.Bold = true;
                        Marshal.ReleaseComObject(range); // Освобождаем Range после использования
                        range = null;
                    }
                    ...
                    for (int row = 0; row < dataTable.Rows.Count; row++)
                    {
                        ...
                        {
                            range = (Excel.Range)worksheet.Cells[row + 2, col + 1]; // Явное приведение к Range
                            range.Value2 = dataTable.Rows[row][col]?.ToString();
                            Marshal.ReleaseComObject(range); // Освобождаем Range
                            range = null;
                        }
                    }
        ...
        private async Task ShowSaleDialogAsync(DataGridViewRow row)
        {
            ...
            btnSave.Click += async (s, e) =>
            {
                try
                {
                    ...
                    int? clientId = cmbClient.SelectedValue != null && cmbClient.SelectedValue != DBNull.Value ? (int?)cmbClient.SelectedValue : null;
                    if (row == null)
                    {
